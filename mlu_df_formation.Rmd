...
```{R}
sarah_data <- map_df(data,function(x) read.CLAN.file(x))
```
...
```{R}
class(new_sarah_data$Speaker)
new_sarah_data$Speaker <- as.factor(new_sarah_data$Speaker)
class(new_sarah_data$Gloss)
class(new_sarah_data$Date)
new_sarah_data$Date <- anytime::anydate(new_sarah_data$Date)
```
...
```{R}
chimot <- filter(new_sarah_data,Speaker=="MOT"|Speaker=="CHI")
...
```{R}
?str_replace_all
chimot$Gloss <- str_replace_all(chimot$Gloss,"[:punct:]","")
```
...
```{R} 
chimot$word_count <- stri_count_words(chimot$Gloss)
...
```{R}
group <- group_by(chimot,Speaker,Date)
```
...
mlu_df <- dplyr::summarise(group, word_sum = sum(word_count),
                                         utterances =n(),
